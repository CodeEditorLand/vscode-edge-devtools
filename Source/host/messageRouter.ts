...
export class MessageRouter {
	...
	constructor(webviewWindow: Window) {
		webviewWindow.addEventListener("DOMContentLoaded", () => {
			...
			this.errorMessageDiv = document.getElementById(
				"error-message",
			) as HTMLElement;
		});
			...
			(messageEvent) => {
				...
				if (!fromExtension) {
					...
					this.onMessageFromFrame(
						messageEvent.data.method as FrameToolsEvent,
						messageEvent.data.args as any[],
					);
	...
	private recordEnumeratedHistogram(
		actionName: string,
		actionCode: number,
		_bucketSize: number,
	): void {
	...
	private recordPerformanceHistogram(
	...
	private fireWebSocketCallback(e: WebSocketEvent, message: string) {
		...
		if (this.toolsFrameWindow && e === "message") {
			this.toolsFrameWindow.postMessage(
				{ method: "dispatchMessage", args: [message] },
				"*",
			);
		}
	}
