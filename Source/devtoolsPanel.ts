...
export class DevToolsPanel {
	...
	private constructor(
		...
		if (this.config.isJsDebugProxiedCDPConnection) {
			this.panelSocket = new JsDebugProxyPanelSocket(
				this.targetUrl,
				(e, msg) => this.postToDevTools(e, msg),
			);
		} else {
		...
		this.panelSocket.on(
			...
			(msg: string) =>
				this.onToggleCSSMirrorContent(msg) as unknown as void,
		);
		...
		this.versionDetectionSocket = new BrowserVersionDetectionSocket(
			this.targetUrl,
		);
	...
	determineVersionFallback() {
		...
		const detectedVersion = () => {
			this.versionDetectionSocket.on(
				"setCdnParameters",
				(msg: { revision: string; isHeadless: boolean }) => {
					this.setCdnParameters(msg);
				},
			);

			this.versionDetectionSocket.detectVersion.bind(
				this.versionDetectionSocket,
			)();
		};
	...
	dispose(): void {
		...
		if (
			...
			vscode.debug.activeDebugSession?.name.includes(
				providedHeadlessDebugConfig.name,
			)
		) {
	...
	private postToDevTools(e: WebSocketEvent, message?: string) {
		...
		if (
			...
			message &&
			message.includes("Runtime.consoleAPICalled")
		) {
	...
	private onSocketMessage(message: string) {
		...
		if (
			message &&
			message.includes('\\"method\\":\\"Overlay.setInspectMode\\"')
		) {
			try {
	...
	private async onSocketOpenInEditor(message: string) {
		...
		const { column, line, url, ignoreTabChanges } = JSON.parse(
			message,
		) as IOpenEditorData;
	...
	private async onSocketCssMirrorContent(message: string) {
		...
		// Parse message and open the requested file
		const { url, newContent } = JSON.parse(
			message,
		) as ICssMirrorContentData;
	...
	private async openEditorFromUri(
		uri: vscode.Uri,
		line?: number,
		column?: number,
	...
	private async parseUrlToUri(url: string): Promise<vscode.Uri | undefined> {
		...
		if (this.config.sourceMaps) {
			sourcePath = applyPathMapping(
				sourcePath,
				this.config.sourceMapPathOverrides,
			);
		}
	...
	static createOrShow(
		...
		if (
			...
			DevToolsPanel.instance.targetUrl === targetUrl
		) {
