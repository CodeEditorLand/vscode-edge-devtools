...
export class CDPTargetsProvider implements vscode.TreeDataProvider<CDPTarget> {
	...
	async getChildren(element?: CDPTarget): Promise<CDPTarget[]> {
		...
		const willShowWorkers = vscode.workspace
				...
				if (responseArray.length > 0) {
					...
					responseArray.forEach((target: IRemoteTargetJson) => {
						...
						if (
							...
							responseIconPromiseArray.push(
								this.downloadFaviconFromSitePromise(
									actualTarget,
								),
							);
						} else if (
							(actualTarget.type !== "service_worker" &&
								actualTarget.type !== "shared_worker") ||
							willShowWorkers
						) {
							targets.push(
								new CDPTarget(
	...
	downloadFaviconFromSitePromise(
		actualTarget: IRemoteTargetJson,
	): Promise<IRemoteTargetJson> {
		...
		const faviconUrl = actualTarget.url.replace(
			faviconRegex,
			"$1favicon.ico",
		);
