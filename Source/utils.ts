...
export const CDN_FALLBACK_REVISION =
	"@f163ae219c3b08cda5aafa6b262442715a8a9893";
 ...
 */
export function getRemoteEndpointSettings(
	...
	if (
		userDataDir === true ||
		(typeof userDataDir === "undefined" && browserPathSet === "Default")
	) {
 ...
 */
export async function getJsDebugCDPProxyWebsocketUrl(
...
export function getCSSMirrorContentEnabled(
	context: vscode.ExtensionContext,
): boolean {
	return context.globalState.get<boolean>("cssMirrorContent") ?? true;
}

export function setCSSMirrorContentEnabled(
	context: vscode.ExtensionContext,
	isEnabled: boolean,
): Thenable<void> {
...
export async function openNewTab(
	...
	port: number,
	tabUrl?: string,
): Promise<IRemoteTargetJson | undefined> {
	try {
...
export function getRuntimeConfig(
	...
	for (const pattern in sourceMapPathOverrides) {
		if (sourceMapPathOverrides.hasOwnProperty(pattern)) {
			...
			resolvedOverrides[replacePattern] =
				replaceWorkSpaceFolderPlaceholder(replacePatternValue);
		}
	}
 ...
 */
export function replaceWebRootInSourceMapPathOverridesEntry(
	webRoot: string,
	entry: string,
): string {
	if (webRoot) {
 ...
 */
export function addEntrypointIfNeeded(
	sourcePath: string,
	defaultEntrypoint: string,
): string {
...
export function applyPathMapping(
	...
	const sortedOverrideKeys = Object.keys(pathMapping).sort(
	...
	for (const leftPattern of sortedOverrideKeys) {
		...
		// Does it match?
		const escapedLeftPattern = debugCore.utils.escapeRegexSpecialChars(
			leftPattern,
			"/*",
		);
 ...
 */
async function verifyFlavorPath(
	flavor: BrowserFlavor | undefined,
	platform: Platform,
): Promise<string> {
	...
	async function findFlavorPath(browserPath: IBrowserPath | undefined) {
		...
		if (
			(await fse.pathExists(browserPath.windows.primary)) &&
...
type ExtensionSettings = [
	string,
	boolean | string | { [key: string]: string } | undefined,
];

export function reportExtensionSettings(
...
export function reportChangedExtensionSetting(
...
export async function reportFileExtensionTypes(
	telemetryReporter: Readonly<TelemetryReporter>,
): Promise<void> {
...
export function checkWithinHoverRange(
	...
	if (position.line >= range.start.line && position.line <= range.end.line) {
		...
		if (
			...
			position.character + 1 < range.start.character
		) {
			return false;
		}
		if (
