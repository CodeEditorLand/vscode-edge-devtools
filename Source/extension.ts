...
import type {
	installFailed,
	showOutput,
} from "vscode-webhint/dist/src/utils/notifications";
...
export function activate(context: vscode.ExtensionContext): void {
			...
			provideHover(document, position) {
				...
				for (const diagnostic of documentDiagnostics) {
					if (
						...
						checkWithinHoverRange(position, diagnostic.range) &&
						(diagnostic.code as DiagnosticCodeType)
					) {
						telemetryReporter.sendTelemetryEvent(
							"user/webhint/hover",
							{
								"hint": (diagnostic.code as DiagnosticCodeType)
									.value,
							},
						);
					}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			`${SETTINGS_STORE_NAME}.attachToCurrentDebugTarget`,
			(
				debugSessionId: string | undefined,
				config: Partial<IUserConfig>,
			): void => {
				void attachToCurrentDebugTarget(
					context,
					debugSessionId,
					config,
				);
			...
			async (fromEmptyTargetView?: boolean) => {
				if (fromEmptyTargetView) {
					telemetryReporter.sendTelemetryEvent("user/buttonPress", {
						"VSCode.buttonCode":
							buttonCode.emptyTargetListLaunchBrowserInstance,
					});
				} else {
					telemetryReporter.sendTelemetryEvent("user/buttonPress", {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			(
				target?: CDPTarget,
				isJsDebugProxiedCDPConnection: boolean = false,
			) => {
				if (!target) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			`${SETTINGS_VIEW_NAME}.openSettings`,
			() => {
				telemetryReporter.sendTelemetryEvent("user/buttonPress", {
			...
			async (target?: CDPTarget) => {
				...
				if (browserInstance) {
					...
					for (const page of browserPages) {
						...
						if (
							!page.isClosed() &&
							normalizedPath === page.target().url()
						) {
							// fire and forget
							void page.close();
							break;
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			() => {
				telemetryReporter.sendTelemetryEvent("user/buttonPress", {
					"VSCode.buttonCode":
						LaunchConfigManager.instance.getLaunchConfig() ===
						"None"
					...
					if (LaunchConfigManager.instance.isValidLaunchConfig()) {
						...
						void vscode.window
							.showErrorMessage(
								...
								autoConfigButtonText,
							)
							.then((value) => {
								if (value === autoConfigButtonText) {
									void LaunchConfigManager.instance
										.configureLaunchJson()
										.then(() =>
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			`${SETTINGS_VIEW_NAME}.launchScreencast`,
			async (fileUri: vscode.Uri): Promise<void> => {
				telemetryReporter.sendTelemetryEvent(
...
export async function launchHtml(fileUri: vscode.Uri): Promise<void> {
	...
	if (!vscode.env.remoteName) {
		...
		void vscode.debug
			...
			.then(() =>
				vscode.debug.startDebugging(undefined, devToolsAttachConfig),
			);
	} else {
...
export async function launchScreencast(
	...
	if (!vscode.env.remoteName) {
		...
		await launchBrowser(
			...
			userDataDir,
			/** headless */ true,
		).then(() => attach(context, url, undefined, true, true));
	}
}
				...
				switch (command) {
					...
					case "vscode-webhint/ignore-browsers-project": {
						if (args.length > 1) {
							...
							telemetryReporter.sendTelemetryEvent(
								"user/webhint/quickfix/ignore-browsers",
								{
									hint: hintName,
									value: browserList.join(","),
								},
							);
						}
						break;
...
export async function attach(
		...
		if (responseArray.length > 0) {
			...
			if (attachUrl) {
				...
				if (
					...
					matchedTargets[0].webSocketDebuggerUrl
				) {
	...
	if (responseArray.length === 0) {
		void ErrorReporter.showErrorDialog({
			...
			message:
				(exceptionStack as string) || "No available targets to attach.",
		});

		telemetryReporter.sendTelemetryEvent(
			"command/attach/error/no_json_array",
			telemetryProps,
		);
	}
}
