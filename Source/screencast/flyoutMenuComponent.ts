...
export default class FlyoutMenuComponent {
	...
	#onClick = () => {
		...
		if (this.#offsetDirection === OffsetDirection.Left) {
			...
			styles.right =
				Math.min(
					document.body.offsetWidth - boundingRect.right,
				).toString() + "px";
		}
			...
			return html`
				<li
					@mousedown=${this.#onItemSelected(
						...
						menuItemSection.onItemSelected,
					)}>
					<i
		...
		return html`
			<button
				...
				${this.#displayCurrentSelection
					? html`${this.#getTitleFromMenuItemSections(
							this.#globalSelectedItem,
						)}`
					: ""}
