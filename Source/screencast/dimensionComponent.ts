...
export default class DimensionComponent {
	...
	#onBlur = (e: Event) => {
		...
		if (e.target === this.textInputWidth.value) {
			this.width = parseInt(
				(this.textInputWidth.value as HTMLInputElement).value,
			);
		} else if (e.target === this.textInputHeight.value) {
			this.height = parseInt(
				(this.textInputHeight.value as HTMLInputElement).value,
			);
		}
	...
	static setDimensionState(
		width: number,
		height: number,
		updateOnResize: boolean,
		disableInputs: boolean,
	) {
		component.#setDimensionState(
			width,
			height,
			updateOnResize,
			disableInputs,
