...
export class Screencast {
	...
	constructor() {
		...
		this.emulationBar = document.getElementById(
			"emulation-bar",
		) as HTMLElement;
				...
				${new FlyoutMenuComponent({
					...
					menuItemSections: [
						...
						},
						{
							...
							menuItems: [
								{
									name: "No prefers-color-scheme emulation",
									value: "",
								},
								{
						...
						},
						{
							...
							menuItems: [
								{
									name: "No forced-colors emulation",
									value: "",
								},
					...
					menuItemSections: [
						{
							...
							menuItems: [
								{
									name: "No vision deficiency emulation",
									value: "none",
								},
								{
									name: "Blurred vision",
		...
		this.cdpConnection.registerForEvent(
			...
			(result) => this.onToggleInspect(result),
		);
	...
	private onDeviceSelected = (value: string) => {
		...
		if (isResponsive) {
			...
			if (device.modes) {
				...
				this.emulatedWidth =
					device.screen[
						defaultDeviceMode.orientation as
							| "horizontal"
							| "vertical"
					].width;
				this.emulatedHeight =
	...
	private updateScreencast(): void {
		const screencastParams = {
			...
			maxHeight: Math.floor(
				this.emulatedHeight * window.devicePixelRatio,
			),
		};
		this.cdpConnection.sendMessageToBackend(
